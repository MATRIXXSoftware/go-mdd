...
func TestPhoneNoValue(t *testing.T) {
	data := []byte("*1234567890123#")
	value, err := decodePhoneNoValue(data)
	assert.Nil(t, err)
	assert.Equal(t, "*1234567890123#", value)

	encoded, err := encodePhoneNoValue(value)
	assert.Nil(t, err)
	assert.Equal(t, data, encoded)
}

func TestPhoneNoListValue(t *testing.T) {
	data := []byte("{*1234567890123#,*1234567890124#}")
	list, err := decodeListValue(data, decodePhoneNoValue)
	assert.Nil(t, err)
	assert.Equal(t, 2, len(list))
	assert.Equal(t, "*1234567890123#", list[0])
	assert.Equal(t, "*1234567890124#", list[1])

	encoded, err := encodeListValue(list, encodePhoneNoValue)
	assert.Nil(t, err)
	assert.Equal(t, data, encoded)
}
...
func TestObjectIDValue(t *testing.T) {
	data := []byte("1:5:1:201")
	value, err := decodeObjectIDValue(data)
	assert.Nil(t, err)
	assert.Equal(t, "1:5:1:201", value)

	encoded, err := encodeObjectIDValue(value)
	assert.Nil(t, err)
	assert.Equal(t, data, encoded)
}

func TestObjectIDListValue(t *testing.T) {
	data := []byte("{1:5:1:201,1:5:1:202}")
	list, err := decodeListValue(data, decodeObjectIDValue)
	assert.Nil(t, err)
	assert.Equal(t, 2, len(list))
	assert.Equal(t, "1:5:1:201", list[0])
	assert.Equal(t, "1:5:1:202", list[1])

	encoded, err := encodeListValue(list, encodeObjectIDValue)
	assert.Nil(t, err)
	assert.Equal(t, data, encoded)
}
